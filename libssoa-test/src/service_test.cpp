...
{
public:
    MyService(ServiceSignature signature) :
        Service(std::move(signature))
    {
    }
    void getConstBuffers() {
        Service::getConstBuffers();
    }
};
...
BOOST_AUTO_TEST_SUITE(service)
    ...
    {
        MyService m("RotateImage(in int, in double)");
    ...
    {
        MyService m("RotateImage(in string)");
        string text("hello");
        BOOST_CHECK_THROW(m.getConstBuffers(), std::logic_error);
    ...
    {
        MyService m("RotateImage(in buffer)");
    ...
    {
        MyService m("RotateImage(in int, in double, out double)");
        BOOST_CHECK_THROW(m.getConstBuffers(), std::logic_error);
    ...
    {
        MyService m("RotateImage(in string, out double)");
        BOOST_CHECK_THROW(m.getConstBuffers(), std::logic_error);
        string text("hello");
    ...
    {
        MyService m("RotateImage(in buffer, out double)");
        BOOST_CHECK_THROW(m.getConstBuffers(), std::logic_error);
